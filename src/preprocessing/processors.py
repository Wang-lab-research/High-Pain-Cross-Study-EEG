...
class Subject:
    ...
    """
    def __init__(self, CFGLog: Dict, subject_id: str, group: str):
        assert isinstance(subject_id, str), "Subject ID must be a string"
        self.subject_id = subject_id
        self.group = group
        self.config = Config.from_json(CFGLog)
        self.parameters = self.config.parameters
        self.output = f"{self.config.output.output_path}/{group}/{subject_id}"
        if not os.path.exists(self.output):
            os.makedirs(self.output)
        
    def __str__(self):
        # tabulate 
        subject_info = [self.subject_id, self.group]
        subject_info = [str(el) for el in subject_info]
        subject_info = "\t".join(subject_info)
    ...
    def get_cleaned_evoked(self):
        pass

    def get_stc_resting(self):
        pass

    def get_stc_evoked(self):
        pass
